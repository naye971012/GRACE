...
class Flamingo(nn.Module):
    ...
    def forward(
        ...
        if vision_x==None:
            ...
            self._encode_vision_x(vision_x=vision_x) #cache처럼 빠르게 미리 gated attention 위해 저장
            self._condition_media_locations(input_ids=lang_x) #image token 위치 확인
        
        #print("CV end",time.time())
